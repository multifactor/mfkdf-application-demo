...
class Dashboard extends React.Component {
  ...
  async componentDidMount() {
    ...
    const res = await axios.post(
      "/api/passwords/list?email=" + encodeURIComponent(this.props.user.email),
      {
        auth,
        time,
      }
  ...
  choose(site) {
    return () => {
      const valid =
        this.pass.current.value.length > 0 &&
        this.user.current.value.length > 0;
  ...
  create(e) {
    ...
    (async () => {
      ...
      const authKey = (await this.props.user.key.ISO9798CCFKey()).toString(
        "hex"
      );
      const time = Date.now();
      const auth = await SHA256(authKey + time);
      ...
      axios
        ...
        .then((res) => {
          this.setState({
            loading: false,
            data: this.state.data.concat([account]),
          });
        })
        .catch((err) => {
          const msg =
  ...
  render() {
    ...
    return (
      <>
        <nav className="navbar navbar-expand-lg fixed-top navbar-dark bg-dark">
          <div className="container">
            <Link to="/" className="navbar-brand">
              <img src={logo} alt="MFKDF" height="30" />
              &nbsp; MFKDF Demo
            </Link>
            <button
            ...
            >
              <ul className="navbar-nav mb-2 mb-lg-0 ms-auto">
                <li className="nav-item dropdown">
                  ...
                  >
                    ...
                    <li>
                      <Link to="/changetotp" className="dropdown-item">
                        Update TOTP
                      </Link>
                    </li>
                  </ul>
              ...
              <form>
                ...
                >
                  <i className="fa fa-plus"></i>&nbsp; Add Password
                </button>
              </form>
            </div>
          </div>
        ...
        <div
          className={
            this.state.modal ? "modal fade show d-block" : "modal fade"
          }
        >
                      ...
                      <div className="site-chosen">
                        <div className="input-group m-0">
                          <span className="input-group-text">
                            <i className="fa fa-search fa-fw invisible" />
                            <img
                              src={this.state.site.logo + "?size=24"}
                              alt={this.state.site.domain}
                            />
                          </span>
                          <input
                            value={
                          ...
                          <button
                            ...
                            type="button"
                            onClick={() => {
                              this.setState({ site: false, valid: false });
                            }}
                          >
                    ...
                    <>
                      ...
                      {this.state.autocomplete && (
                        <div className="list-group">
                          {this.state.autocomplete.map((site) => (
                            <button
                              key={site.domain}
                              type="button"
                              onClick={this.choose(site)}
                              className="list-group-item list-group-item-action"
                            >
                              <div className="row align-items-center">
                                <div className="col-1">
                                  <img
                                    src={site.logo + "?size=24"}
                                    alt={site.domain}
                                  />
                                </div>
