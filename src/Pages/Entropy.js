...
class Entropy extends React.Component {
  render() {
    ...
    const pbkdf2EntropyBits = Math.log2(inner.outputs.password.strength.guesses);
      ...
      <div className="form text-center wide">
        ...
        <div className="card text-start">
          <div className="position-relative">
            <div className="arrow-circle"><i className="fa fa-arrow-right"></i></div>
            ...
            <div className="row text-center">
              <div className="col-6">
                <p><b>Password Derived Key: {Math.round(pbkdf2EntropyBits)} bits</b></p>
                <p><i>That would take an attacker about {timeAgo.format(Date.now() + (2**pbkdf2EntropyBits)).replace('in', '')} to crack at 1,000 attempts per second.</i></p>
              </div>
              <div className="col-6">
                <p><b>Multi-Factor Derived Key: {Math.round(mfkdfEntropyBits)} bits</b></p>
                <p><i>That would take an attacker about {timeAgo.format(Date.now() + (2**mfkdfEntropyBits)).replace('in', '')} to crack at 1,000 attempts per second.</i></p>
              </div>
            </div>
          </div>
          <Link to="/dashboard"><button className="mt-3 btn btn-success w-100 m-0">Finish Setup &nbsp;<i className="fa fa-check-circle"></i></button></Link>
        </div>
      </div>
